...
"""
import logging
from typing import Callable, List, Dict, Any, Optional, Union, Protocol
...
class SQLBatcher:
    ...
    exceeding query size limits while optimizing for performance. It's particularly
    useful for systems like Trino, Spark SQL, Snowflake, and other SQL engines
    that have query size limitations.
    ...
    def __init__(self, max_bytes: int = 700_000, delimiter: str = ";\n", dry_run: bool = False):
        ...
        Args:
            max_bytes: Maximum size in bytes of each statement batch (default: 700,000)
                For Trino, a common limit is around 1MB, so 700KB is a safe default.
                For other engines, adjust according to their specific limits.
    ...
    def reset(self) -> None:
        """Reset the current batch, clearing all queued statements."""
        self.current_batch = []
        self.current_size = 0
    ...
    def add_statement(self, sql: str) -> bool:
        ...
        This method calculates the byte size of the statement and adds it to the
        current batch if it fits within the size limit. If adding this statement
        would exceed the batch size limit, it returns True to indicate the batch
    ...
    def flush(self, execute_callback: Callable[[str], Any], 
             query_collector: Optional[Union[QueryCollector, Any]] = None,
             metadata: Optional[Dict[str, Any]] = None) -> None:
        """
        Flush the current batch using the provided callback.
        
        This method executes all statements in the current batch using the provided
        callback function. It keeps track of successes and handles errors appropriately.
    ...
    def process_statements(self, sql_statements: List[str], 
        ...
        This is the main entry point for processing a list of SQL statements efficiently.
        It automatically handles batching statements based on size limits and executes
        them through the provided callback.
